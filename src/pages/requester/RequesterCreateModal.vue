...
function addItem(item: {[x: string]: any }): void {
    itemOrder.value.push(item);
}
</script>
<template>
    <div class="modal" :class="{ 'modal-open': requester.create }">
        <div class="modal-box w-8/12 max-w-5xl">
            <section class="max-w-4xl my-6 p-6 mx-auto bg-white rounded-md shadow-md">
                ...
                <Form @submit="addItem" :validation-schema="itemInOrder">
                    <div>
                        <label class="label">
                            <span class="label-text text-xl">select item</span>
                        </label>
                        <Field class="select select-bordered w-full max-w-xs m-3" name="item" as="select"
                            placeholder="item">
                            <option disabled selected>Select item</option>
                            <option v-for="item in itemLists" :value="item" @click="addItem(item)">
                                {{ item.name }}
                            </option>
                        </Field>
                        <button class="btn btn-primary">add</button>
                        <ErrorMessage name="item" class="text-red-500" />
                    </div>
                    <div>{{ itemOrder }}</div>
                </Form>
            </section>
